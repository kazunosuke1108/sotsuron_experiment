...
#         video=cv2.VideoWriter(mp4_dir_path+"/"+video_basename[:-4]+".mp4",fourcc, 6.0,size)
#         if cap.isOpened():
#             while True:
#                 if ret:
...
for i, videoPath in enumerate(mp4_videos):
    ...
    fourcc = cv2.VideoWriter_fourcc('m','p','4', 'v')
    writer = cv2.VideoWriter(skeleton_dir_path+"/"+video_basename[:-4]+"_skeleton.mp4",fourcc, fps, (width, height))
