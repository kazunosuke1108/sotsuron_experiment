...
class AnalyzeEigenFreq():
    ...
    def load_data(self):
        ...
        try:
            ...
            start_timestamp=1.707029425737472057e+09
            end_timestamp=start_timestamp+5
    ...
    def plot_all_imu(self):
        ...
        gs1_start_timestamp=self.base_accurate_imu_data["timestamp"].min()+2.25
        gs1_end_timestamp=self.base_accurate_imu_data["timestamp"].max()-1+0.25
        temp_base_accurate_imu_data=self.base_accurate_imu_data[self.base_accurate_imu_data["timestamp"]>=gs1_start_timestamp]
        temp_base_accurate_imu_data=temp_base_accurate_imu_data[temp_base_accurate_imu_data["timestamp"]<=gs1_end_timestamp]
        temp_head_imu_data=self.head_imu_data[self.head_imu_data["timestamp"]>=gs1_start_timestamp]
        temp_head_imu_data=temp_head_imu_data[temp_head_imu_data["timestamp"]<=gs1_end_timestamp]
        temp_zed_imu_data=self.zed_imu_data[self.zed_imu_data["timestamp"]>=gs1_start_timestamp]
        temp_zed_imu_data=temp_zed_imu_data[temp_zed_imu_data["timestamp"]<=gs1_end_timestamp]
        plt.plot(temp_base_accurate_imu_data["timestamp"],temp_base_accurate_imu_data["lin_acc_x"],label=r"base $\it{\alpha_x}$")
        plt.plot(temp_head_imu_data["timestamp"],-temp_head_imu_data["lin_acc_x_calib"],label=r"head $\it{\alpha_x}$")
        plt.plot(temp_zed_imu_data["timestamp"],temp_zed_imu_data["lin_acc_x_calib"],label=r"zed $\it{\alpha_x}$")
        plt.grid()
        plt.legend()
        plt.xlabel("Time $\it{t}$ [s]")
        plt.ylabel("Acceleration $\it{a}$ [m/s$^{2}$]")
        plt.title("all IMU"+" "+os.path.basename(self.results_dir_path))
