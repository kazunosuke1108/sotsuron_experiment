...
try:
    ...
    csvpath="C:/Users/hayashide/kazu_ws/sotsuron_experiment/sotsuron_experiment/results/_2023-12-16-17-47-38/_2023-12-16-17-47-38_tf_raw.csv"
...
# リサンプリング
data=resampling_processor(data,"1S")
# resample_dt_str="1S"
...
plt.plot(data["timestamp"][data["timestamp"]<data["timestamp"].values[0]+30],data["gravity_vx"][data["timestamp"]<data["timestamp"].values[0]+30],label="gravity_vx")
# data_timestamp=data["timestamp"]
data=mean_processor(data,span=50)
plt.xlabel("Time $\it{t}$ [s]")
plt.ylabel("Velocity $\it{v_x}$ $\it{v_y}$ [m/s]")
plt.grid()
# data["timestamp"]=data_timestamp

# plt.plot(data["timestamp"][data["timestamp"]<data["timestamp"].values[0]+30],data["gravity_vx"][data["timestamp"]<data["timestamp"].values[0]+30])
plt.savefig(os.path.split(csvpath)[0]+"/"+os.path.basename(csvpath)[:-8]+"_vel_resamplingOnly.png")


def fft_processor(data):
    # NaNの削除
    ...
    print(data)
    ax1.plot(data["timestamp"][data["timestamp"]<data["timestamp"].values[0]+30],data["gravity_x"][data["timestamp"]<data["timestamp"].values[0]+30],"r-",label="gravity_x")
    ax1.plot(data["timestamp"][data["timestamp"]<data["timestamp"].values[0]+30],data["gravity_y"][data["timestamp"]<data["timestamp"].values[0]+30],"b-",label="gravity_y")
    ...
    ax2 = ax1.twinx()
    ax2.plot(data["timestamp"][data["timestamp"]<data["timestamp"].values[0]+30],data["gravity_vx"][data["timestamp"]<data["timestamp"].values[0]+30],"m--",linewidth=0.2,label="raw velocity $\it{v_x}$")
    ax2.plot(data["timestamp"][data["timestamp"]<data["timestamp"].values[0]+30],data["gravity_vy"][data["timestamp"]<data["timestamp"].values[0]+30],"c--",linewidth=0.2,label="raw velocity $\it{v_y}$")
    ax2.plot(data["timestamp"][data["timestamp"]<data["timestamp"].values[0]+30],data["gravity_vx_filtered"][data["timestamp"]<data["timestamp"].values[0]+30],"m",label="FFT velocity $\it{v_x}$")
    ax2.plot(data["timestamp"][data["timestamp"]<data["timestamp"].values[0]+30],data["gravity_vy_filtered"][data["timestamp"]<data["timestamp"].values[0]+30],"c",label="FFT velocity $\it{v_y}$")
