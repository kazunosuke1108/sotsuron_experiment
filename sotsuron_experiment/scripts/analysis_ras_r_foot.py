...
data=data[data["timestamp"]<data["timestamp"].values[0]+35]
data=resampling_processor(data,"0.01S")
...
# クラスターの絞り込み
clusterdata=clusterdata[clusterdata["cluster_size"]>5]
...
try:
    ...
    clusterdata["adjacent_vel_x"]=(clusterdata["mean_x"].values[1:]-clusterdata["mean_x"].values[:-1])/(clusterdata["start_timestamp"].values[1:]-clusterdata["end_timestamp"].values[:-1])
...
try:
    ...
    clusterdata["stride"]=abs(clusterdata["mean_x"].values[1:]-clusterdata["mean_x"].values[:-1])
print(clusterdata)
